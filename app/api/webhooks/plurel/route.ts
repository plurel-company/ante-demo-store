/** POST /api/webhooks/plurel — verify Plurel Pay webhook signature; fulfill on group.funded. */
import {
  listWebhookSecrets,
...
export async function POST(req: Request) {
  ...
  if (event.type === "group.funded") {
    ...
    if (!result.ok) {
      if (result.status === 400 && result.error === "Missing order_ref") {
        console.error("[plurel webhook] group.funded missing order_ref", event.data);
      } else if (result.status === 400 && result.error === "Missing or invalid total") {
        console.error("[plurel webhook] group.funded missing or invalid total", event.data);
      } else if (result.status === 404) {
        console.error("[plurel webhook] group.funded unknown order_ref", {
          orderRef: event.data.order_ref,
        });
      } else if (result.status === 401) {
        console.error("[plurel webhook] group.funded credential mode mismatch", {
          orderRef: event.data.order_ref,
          verifiedMode,
        });
      }
    ...
    if (result.duplicate) {
      console.info("[plurel webhook] group.funded duplicate", {
      ...
      console.info("[plurel webhook] group.funded", {
        orderRef: result.order.orderRef,
        sessionId: result.order.sessionId,
  ...
  console.info("[plurel webhook]", event.type, event.data);
